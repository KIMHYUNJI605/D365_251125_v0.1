...
export const Card: React.FC<CardProps> = ({ title, action, children, className = 'bg-white', footer, fullHeight = true }) => {
  ...
  return (
    <div className={combinedClass}>
      {(title || action) && (
        <div className="px-4 py-3 md:px-6 md:py-5 border-b border-slate-100/50 flex justify-between items-center shrink-0">
          {title && <h3 className="text-base md:text-lg font-bold text-slate-800 leading-tight tracking-tight">{title}</h3>}
          {action && <div>{action}</div>}
        </div>
      )}
      <div className="p-4 md:p-6 flex-1 overflow-auto no-scrollbar relative">
        {children}
      </div>
      {footer && (
        <div className="px-4 py-3 md:px-6 md:py-4 border-t border-slate-100 bg-white/50 shrink-0">
          {footer}
        </div>
      )}
    </div>
